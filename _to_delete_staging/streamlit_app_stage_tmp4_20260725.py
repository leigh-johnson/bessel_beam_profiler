...
def ray_diagram(p, geo):
    ...
    ax.annotate(r"$\alpha_1$", (z1 - 1.6 * dx, 0.55 * H), ha="right", **lab)
    ax.annotate(r"$\alpha_2$", (z2 - 1.6 * dx, 0.40 * H), ha="right", **lab)
    ax.annotate(r"$\alpha_3$", (z3p + 1.6 * dx, -0.45 * H), **lab)
    b1 = geo["betas"][0]
    xb1 = z1 + min(0.05 * span, 0.30 * H * mm / b1)   # keep the drop on-plot
    ax.plot([z1, xb1], [w / mm, w / mm], ls=":", color="0.55", lw=0.8)
    ax.annotate(r"$\beta_1$", (xb1 + 0.004 * span,
                               (w - 0.55 * b1 * (xb1 - z1)) / mm), **lab)

    if geo["ok"]:
        xb3 = z3p + 0.07 * span
        ax.plot([z3p, xb3], [geo["rho_out"] / mm] * 2, ls=":", color="0.55",
                lw=0.8)
        ax.annotate(r"$\beta_3$", (xb3 + 0.004 * span,
                                   (geo["rho_out"]
                                    - 0.55 * geo["th_end"] * (xb3 - z3p)) / mm),
                    **lab)
        ax.annotate(r"$\theta$", (z3p + geo["z_start"] - 0.055 * span,
                                  0.30 * geo["th_end"] * 0.055 * span / mm),
                    **lab)
        for xr, rr, s in ((z3p + 0.018 * span, geo["rho_in"], r"$\rho_{in}$"),
                          (z3p + 0.036 * span, geo["rho_out"],
                           r"$\rho_{out}$")):
            ax.annotate("", (xr, rr / mm), (xr, 0),
                        arrowprops=dict(arrowstyle="<->", color="0.35", lw=0.8))
            ax.annotate(s, (xr + 0.004 * span, 0.45 * rr / mm), **lab)
